...
QString veriPushButtonStyleSheet = QString("QPushButton { background: " + STR_COLOR + "; width: %1px; height: %2px; border: none; color: white} \
                            QPushButton:disabled { background: #EBEBEB; color: #E30011; } \
...
void setupAmountWidget(QLineEdit *widget, QWidget *parent)
{
    ...
    amountValidator->setDecimals(BitcoinUnits::decimals(BitcoinUnits::LEC));
...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitcoinUnits::parse(BitcoinUnits::LEC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
{
    // Convert lecoin:// to lecoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("lecoin://"))
    {
        uri.replace(0, 12, "lecoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "LeCoin.lnk";
}
...
{
    return GetAutostartDir() / "lecoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("LeCoin-Qt") + " " + tr("version") + " " +
