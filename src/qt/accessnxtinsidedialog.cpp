    ...
    model(0)
{
    ...
    ui->addressIn_AN->setPlaceholderText(tr("输入乐币地址（例如LTHZfUg11wEJmSgBLUcmCKGYekHFEdfjs）"));
#endif
...
void AccessNxtInsideDialog::on_accessNxtInsideButton_AN_clicked()
{
    ...
    if (!addr.IsValid())
    {
        ...
        ui->statusLabel_AN->setText(tr("输入的地址是无效.") + QString(" ") + tr("请检查地址，然后再试一次."));
        return;
    }
    CKeyID keyID;
    if (!addr.GetKeyID(keyID))
    {
        ui->addressIn_AN->setValid(false);
        ui->statusLabel_AN->setStyleSheet("QLabel { color: red; }");
        ui->statusLabel_AN->setText(tr("输入的地址错误，请检查.") + QString(" ") + tr("请检查地址，然后再试一次."));
        return;
    }
    ...
    {
        ui->statusLabel_AN->setStyleSheet("QLabel { color: red; }");
        ui->statusLabel_AN->setText(tr("钱包解锁被取消."));
        return;
    }
    ...
    {
        ui->statusLabel_AN->setStyleSheet("QLabel { color: red; }");
        ui->statusLabel_AN->setText(tr("私钥输入地址错误."));
        return;
    }
    ...
    QUrl url = QUrl("http://localhost:7872");
