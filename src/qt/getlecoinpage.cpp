...
GetlecoinPage::GetlecoinPage(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::GetlecoinPage),
    walletModel(0)
{
    ui->setupUi(this);
    ...
    if (fNoHeaders)
        ...
        GUIUtil::header(this, QString(":images/headerGetlecoinSmall"));
    else
        GUIUtil::header(this, QString(":images/headerGetlecoin"));
    this->layout()->setContentsMargins(0, HEADER_HEIGHT, 0, 0);

    CookieJar *getlecoinJar = new CookieJar;
    ui->webView->page()->networkAccessManager()->setCookieJar(getlecoinJar);
...
GetlecoinPage::~GetlecoinPage()
{
    delete ui;
}

void GetlecoinPage::setModel(WalletModel *model)
{
    this->walletModel = model;

    QUrl url(QString(walletUrl).append("free/"));
