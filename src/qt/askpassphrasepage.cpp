    ...
    fCapsLock(false)
{
    ...
    switch(mode)
    {
        case Encrypt: // Ask passphrase x2
            ...
            ui->passEdit2->setPlaceholderText(tr("新密码..."));
            ui->passEdit3->setPlaceholderText(tr("再输入一次新密码"));
            ui->messageLabel->setText(tr("欢迎您！请输入10个或更多的字符，或8个或更多字的密码."));
            ui->warningLabel->setText(tr("警告：如果你失去了你的密码，你将失去所有的硬币!"));
            break;
        case Lock: // Ask passphrase
            ui->passEdit1->hide();
            ui->passEdit2->hide();
            ui->passEdit3->hide();
            ui->messageLabel->setText(tr("单击确定以锁定钱包."));
            break;
        case Unlock: // Ask passphrase
            ...
            ui->passEdit1->setPlaceholderText(tr("请输入乐币钱包密码"));
            ui->messageLabel->setText(tr("请输入您的密码来解锁钱包."));
            break;
    }
...
void AskPassphrasePage::accept()
{
    ...
    case Encrypt: // Encrypt wallet and set password
        ...
        {
            ui->messageLabel->setText(tr("新密码不能为空."));
            break;
        }
        if(newpass1.length() < 10)
        {
            ui->messageLabel->setText(tr("新的口令必须为10或更多个字符."));
            break;
        }
        if(newpass1 != newpass2)
        {
            ui->messageLabel->setText(tr("该密码不正确."));
            break;
        }
        else
        {
            ...
            {
                ui->messageLabel->setText(tr("乐币现在将重新启动以完成加密过程."));
                ui->warningLabel->setText(tr("重要提示：你的钱包以前的备份应更换新的密码."));
                ui->passEdit2->setText(QString(" ").repeated(ui->passEdit2->text().size()));
            ...
            {
                ui->messageLabel->setText(tr("钱包加密失败，原因是内部错误。你的钱包没有加密."));
            }
        }
        break;
    ...
    case Unlock: // Turn on staking
        ...
        {
            ui->messageLabel->setText(tr("钱包输入的密码不正确."));
        }
        else
        {
            // Attempt to overwrite text so that they do not linger around in memory
...
bool AskPassphrasePage::event(QEvent *event)
{
    ...
    if (event->type() == QEvent::KeyPress) {
        ...
        if (fCapsLock) {
            ui->warningLabel->setText(tr("警告：大写键关闭!"));
        } else {
    ...
    if (event->type() == QEvent::KeyPress) {
        ...
        if (str.length() != 0) {
            ...
            if ((fShift && psz->isLower()) || (!fShift && psz->isUpper())) {
                fCapsLock = true;
                ui->warningLabel->setText(tr("警告：大写键开启!"));
