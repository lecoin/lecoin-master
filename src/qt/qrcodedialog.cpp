...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non LEC unit input in lnReqAmount, we generate the URI with LEC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::formatMaxDecimals(BitcoinUnits::LEC, ui->lnReqAmount->value(), BitcoinUnits::maxdecimals(BitcoinUnits::LEC)));
            paramCount++;
        }
        else
        {
