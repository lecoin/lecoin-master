...
QString TransactionTableModel::formatTxStatus(const TransactionRecord *wtx) const
{
    ...
    case TransactionStatus::OpenUntilDate:
        status = tr("直到 %1").arg(GUIUtil::dateTimeStr(wtx->status.open_for));
        break;
    ...
    case TransactionStatus::Confirming:
        status = tr("确认中 (推荐确认数 %2 已确认 %1").arg(wtx->status.depth).arg(TransactionRecord::RecommendedNumConfirmations);
        break;
    ...
    case TransactionStatus::Immature:
        status = tr("未成熟 (%1 个确认, 将在 %2 个数据块后进入可支配状态").arg(wtx->status.depth).arg(wtx->status.depth + wtx->status.matures_in);
        break;
    case TransactionStatus::MaturesWarning:
        status = tr("没有收到任何其他节点此块，将可能不被接受!");
        break;
