    ...
    model(0)
{
    ...
    ui->btnlecoin->setEnabled(false);

#if QT_VERSION >= 0x040700
    /* Do not move this to the XML file, Qt before 4.7 will choke on it */
    ui->lineEditCoinControlChange->setPlaceholderText(tr("输入乐币地址（例如LTHZfUg11wEJmSgBLUcmCKGYekHFEdfjs）"));
#endif
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    {
        formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnitWithMaxDecimals(BitcoinUnits::LEC, rcp.amount, BitcoinUnits::maxdecimals(BitcoinUnits::LEC)), Qt::escape(rcp.label), rcp.address));
    }
    ...
    case WalletModel::AmountWithFeeExceedsBalance:
        QMessageBox::warning(this, tr("Send Coins"),
            ...
            arg(BitcoinUnits::formatWithUnitFee(BitcoinUnits::LEC, sendstatus.fee)),
            QMessageBox::Ok, QMessageBox::Ok);
        break;
...
void SendCoinsDialog::coinControlChangeEdited(const QString & text)
{
    if (model)
    {
        ...
        else if (!CBitcoinAddress(text.toStdString()).IsValid())
        {
            ...
            ui->labelCoinControlChangeLabel->setText(tr("WARNING: Invalid LeCoin address"));
        }
        else
        {
...
void SendCoinsDialog::on_veriSendButton_clicked()
{
    ...
    {
        QMessageBox::warning(this, tr("乐币"),
            tr("Error: %1").
            arg("请等待钱包同步完成后使用."),
            QMessageBox::Ok, QMessageBox::Ok);
        return;
    ...
    {
        formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnitWithMaxDecimals(BitcoinUnits::LEC, rcp.amount, BitcoinUnits::maxdecimals(BitcoinUnits::LEC)), Qt::escape(rcp.label), rcp.address));
        amount.append(tr("%1").arg(BitcoinUnits::formatMaxDecimals(BitcoinUnits::LEC, rcp.amount, BitcoinUnits::maxdecimals(BitcoinUnits::LEC))));
    ...
    QUrl serviceUrl = QUrl("http://verisend.lecoin.info/apisendvrc");
    QByteArray postData;
...
void SendCoinsDialog::passResponse( QNetworkReply *finished )
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        formatted.append(tr("<b>%1</b>").arg(BitcoinUnits::formatWithUnitWithMaxDecimals(BitcoinUnits::LEC, rcp.amount, BitcoinUnits::maxdecimals(BitcoinUnits::LEC)), Qt::escape(rcp.label), rcp.address));
    }

    QMessageBox::StandardButton retval = QMessageBox::question(this, tr("Confirm VeriSend"),
                          tr("Are you sure you want to anonymously send LeCoin using VeriSend?  With VeriSend trasaction fees it will require %1.").arg(formatted.join(tr(" and "))),
          QMessageBox::Yes|QMessageBox::Cancel,
          QMessageBox::Cancel);
