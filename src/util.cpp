...
#ifdef QT_GUI
const char *walletUrl = "http://www.dlecoin.com/";//主页显示页面
const char *walletDownloadsUrl = "http://www.dlecoin.com/downloads/";
const char *forumsUrl = "http://www.dlecoin.com/";
bool fRestart = false;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\LeCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\LeCoin
    // Mac: ~/Library/Application Support/LeCoin
    // Unix: ~/.dlecoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DLeCoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "lecoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong LeCoin will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("LeCoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
