...
void RestartWallet(const char *parm, bool fOldParms)
{
    ...
    if (fNewVersion && !fBootstrapTurbo && !fRescan && !fEncrypt) // fNewVersion could be true while trying to do other restarts
    {
        ...
        command = QString(GetDataDir().string().c_str()) + QString("/") + QString(GetArg("-vFileName","lecoin-setup.exe").c_str());
#else
#ifdef MAC_OSX
        ...
        newArgv.append(QString(GetDataDir().c_str()) + QString("/") + QString(GetArg("-vFileName","lecoin-setup.pkg").c_str()));
#else
        // If Linux, just restart (already extracted lecoin-qt from the zip in downloader.cpp).
        parm = NULL;
        fOldParms = false;
        newArgv.clear();
        // Installer created by makeself.sh
        command = QString(GetDataDir().c_str()) + QString("/") + QString(GetArg("-vFileName","lecoin-setup.run").c_str());
        newArgv.append(QString("--target"));
        newArgv.append(QString(GetProgramDir().c_str()));
        // Make executable
        boost::filesystem::path installer(GetDataDir() / GetArg("-vFileName","lecoin-setup.run"));
...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("LeCoin Version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  lecoind [options]                     " + "\n" +
                  "  lecoind [options] <command> [params]  " + _("Send command to -server or lecoind") + "\n" +
                  "  lecoind [options] help                " + _("List commands") + "\n" +
                  "  lecoind [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "lecoin:"))
                fCommandLine = true;
...
{
    uiInterface.ThreadSafeMessageBox(str, _("LeCoin"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("LeCoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file path (default: <cwd>/lecoin.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: lecoind.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 13556 or testnet: 10556)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 23557 or testnet: 20557)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  LeCoin is probably already running."), strDataDir.c_str()));
    ...
    if (fDaemon)
        fprintf(stdout, "LeCoin server starting\n");
    ...
    if (filesystem::exists(GetDataDir() / strWalletFileName))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("LeCoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        }
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of LeCoin") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart LeCoin to complete") << "\n";
